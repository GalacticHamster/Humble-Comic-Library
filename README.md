...
- See a small non-blocking `Checking your library…` indicator while Humble finishes rendering a bundle. The summary panel’s **Diagnostics** disclosure reports local-library read time, detected item count, comparison time, and refresh count, which helps distinguish a slow Humble page from extension work.
